...
// Возвращает то, что было записано для этой дилеммы РАНЬШЕ — то есть до
// текущего прохождения. Используется фичей "Эхо" (сравнение с собой из
// прошлого), поэтому вызывать нужно ДО recordLocalSession, иначе
// возвращаемая запись окажется той же самой, которую вы только что делаете.
export function getPastSession(slug: string): LocalSession | null {
  return loadLocal().find((s) => s.slug === slug) ?? null;
}
